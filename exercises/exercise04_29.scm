    ...
    (assign-exp (var exp1)
      (begin
        (setref!
          ...
          (value-of exp1 env))
        (num-val 27)))
    (arrnew-exp (size exp1)
      (newarray (expval->num (value-of size env))
                (value-of exp1 env)))
    (arrref-exp (arr idx)
      (arrayref (value-of arr env)
                (expval->num (value-of idx env))))
    (arrset-exp (arr idx exp1)
      (arrayset! (value-of arr env)
                 (expval->num (value-of idx env))
                 (value-of exp1 env)))))
