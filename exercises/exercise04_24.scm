...
(define (execute stmt env)
  (cases statement stmt
    ...
    (var-stmt (var* body)
      (if (null? var*)
          (execute body env)
          (execute (var-stmt (cdr var*) body)
                   (extend-env (car var*) (newref 0) env))))
    (read-stmt (var)
      (begin
        (setref!
          (apply-env env var)
...
(assert-eval "var x; {x = 42; halt x}" (num-val 42))
(assert-eval "if zero?(0) halt 5 halt 9" (num-val 5))
(assert-eval "halt not(zero?(0))" (bool-val #f))
(assert-eval "var x; {do-while x = 1 zero?(99); halt x}" (num-val 1))
