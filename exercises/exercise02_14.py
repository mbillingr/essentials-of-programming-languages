...
# Instead of adding another procedure to the list and effectively reimplementing apply_env,
# I'd rather catch the exception :)
def has_binding(env, var):
    try:
        apply_env(env, var)
    except LookupError:
        return False
...
assert has_binding(extend_env("x", 42, empty_env()), "x")
assert not has_binding(empty_env(), "x")
