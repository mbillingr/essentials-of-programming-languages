...
(define the-cps-in-grammar
  '((in-program (in-exp) a-in-program)
    ...
    (in-exp
      ("begin" in-exp (arbno in-exp) "end")
      begin-exp)

    (in-exp
      ("letcc" identifier "in" in-exp)
      letcc-exp)

    (in-exp
      ("throw" in-exp "to" in-exp)
      throw-exp)))
...
(define (cps-of-exp exp k-exp)
  (cases in-exp exp
    ...
    (begin-exp (first rest)
      (cps-of-begin-exp first rest k-exp))
    (letcc-exp (var body)
      (cps-let-exp var k-exp (cps-of-exp body var)))
    (throw-exp (exp1 exp2)
      (cps-of-exps (list exp1 exp2)
        (lambda (simples)
          (make-send-to-cont (cadr simples) (car simples)))))))
...
(define (cps-of-begin-exp first rest k-exp)
  (cps-of-exps (cons first rest)
    (lambda (simples)
      (let make-let ((simples simples))
        (if (null? (cdr simples))
            (simple-exp->exp (car simples))
            (let ((tmp (fresh-identifier 'begin)))
              (cps-let-exp 
                tmp          
                (car simples)
                (make-let (cdr simples)))))))))
...
(assert-eval
  "letcc exit in begin 1 2 throw 42 to exit 3 4 end"
  (num-val 42))
