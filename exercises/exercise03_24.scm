...
(assert-eval
  ...
  let true = zero?(0) in
  let false = zero?(1) in
  let not = proc (b) if b then false else true in
  let make-even = proc (even-maker)
                    proc (odd-maker)
                      proc (x)
                        if zero?(x)
                        then true
                        else (((odd-maker even-maker) odd-maker) -(x,1))
  in let make-odd = proc (even-maker)
                      proc (odd-maker)
                        proc (x)
                          if zero?(x)
                          then false
                          else (((even-maker even-maker) odd-maker) -(x,1))
     in let even = proc (x) (((make-even make-even) make-odd) x)
     in let odd = proc (x) (((make-odd make-even) make-odd) x)
        in (odd 99)
  "
  (bool-val #t))
