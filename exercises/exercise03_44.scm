  ...
  (cases expression exp
    ...
    (let-exp (var exp1 body)
      (let* ((letval (translation-of exp1 senv))
             ...
             (new-body (translation-of body new-env)))
        (if (binding-referenced? 0 new-body)
            (nameless-let-exp letval new-body)
            (shift-references new-body -1 1))))
    (proc-exp (var body)
      (nameless-proc-exp
        (translation-of body
...
(define (binding-referenced? idx exp)
  (cases expression exp
    (const-exp (num) #f)
    (diff-exp (exp1 exp2)
      (or (binding-referenced? idx exp1)
          (binding-referenced? idx exp2)))
    (zero?-exp (exp1)
      (binding-referenced? idx exp1))
    (if-exp (exp1 exp2 exp3)
      (or (binding-referenced? idx exp1)
          (binding-referenced? idx exp2)
          (binding-referenced? idx exp3)))
    (call-exp (rator rand)
      (or (binding-referenced? idx rator)
          (binding-referenced? idx rand)))
    (nameless-var-exp (n) 
      (eqv? n idx))
    (nameless-let-exp (exp1 body)
      (or (binding-referenced? idx exp1)
          (binding-referenced? (+ 1 idx) body)))
