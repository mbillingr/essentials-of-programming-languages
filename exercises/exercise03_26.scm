...
(define (procedure var body env)
  (let* ((fvars (remove var (free-vars body)))
         (fvals (map (lambda (fv) (apply-env env fv)) fvars))
         (cls (extend-env* fvars fvals (empty-env))))
    (lambda (val)
      (value-of body (extend-env var val cls)))))
...
(define (free-vars exp)
  (cases expression exp
    ...
    (diff-exp (exp1 exp2)
      (merge (free-vars exp1) (free-vars exp2)))
    (zero?-exp (exp1)
      (free-vars exp1))
    (if-exp (exp1 exp2 exp3)
      (merge (free-vars exp1) (merge (free-vars exp2) (free-vars exp3))))
    (let-exp (var exp1 body)
      (merge (remove var (free-vars body))
             (free-vars exp1)))
    (proc-exp (var body)
      (remove var (free-vars body)))
    (call-exp (rator rand)
      (merge (free-vars rator) (free-vars rand)))))

(define (remove item set)
  (cond ((null? set) '())
        ((eqv? item (car set)) (cdr set))
        (else (cons (car set) (remove item (cdr set))))))

(define (insert item set)
  (cond ((null? set) (list item))
        ((eqv? item (car set)) set)
        (else (cons (car set) (insert item (cdr set))))))

(define (merge set1 set2)
  (if (null? set1)
      set2
      (merge (cdr set1) (insert (car set1) set2))))
