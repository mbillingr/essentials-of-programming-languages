...
;  - read statements
;  - do-while statements
;  - initialized var blocks (letrec style)
;  - subroutines
...
(define (execute stmt env)
  (cases statement stmt
    ...
    (var-stmt (var* exp* body)
      (let ((recenv 
              (extend-env* var* 
                           (map (lambda (v) (newref 0)) var*) 
                           env)))
        (map (lambda (var exp) 
               (execute (assign-stmt var exp) recenv)) 
             var* exp*)
        (execute body recenv)))
    (read-stmt (var)
      (begin
    ...
    (call-stmt (rator rand)
      (let ((proc (expval->subr (value-of rator env)))
...
(define (value-of exp env)
  (cases expression exp
    ...
    (subr-exp (var body)
      (subr-val (subroutine var body env)))
    (call-exp (rator rand)
...
(define (subroutine var body env)
  (lambda (val)
    (execute body (extend-env var (newref val) env))))
