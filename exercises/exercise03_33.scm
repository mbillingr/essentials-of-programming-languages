...
(define-datatype environment environment?
  ...
  (extend-env-rec (p-name* (list-of symbol?)) (b-var** (list-of (list-of symbol?))) (body* (list-of expression?)) (env environment?)))
...
(assert-eval
  "letrec 
     ...
     times(x y) = if zero?(x) then 0 else -((times -(x,1) y), -(0,y))
   in (odd (times 7 9))"
  (num-val 1))
